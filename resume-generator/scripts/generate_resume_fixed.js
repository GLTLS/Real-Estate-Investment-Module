...
function analyzeJobRequirements(jobDescription) {
  ...
  if (yearMatch) {
    ...
    analysis.positionMatch.push(`✅ ${yearMatch[1]}+ years experience - matches requirement`);
  }
  
  // 检查技能要求
  const skillKeywords = [
    { keyword: 'python', display: 'Python' },
    { keyword: 'sas', display: 'SAS' },
    { keyword: 'sql', display: 'SQL' },
    { keyword: 'qlik', display: 'Qlik' },
    { keyword: 'power bi', display: 'Power BI' },
    { keyword: 'tableau', display: 'Tableau' },
    { keyword: 'ai', display: 'AI' },
    { keyword: 'machine learning', display: 'Machine Learning' },
    { keyword: 'big data', display: 'Big Data' },
    { keyword: 'data mining', display: 'Data Mining' },
    { keyword: 'statistical', display: 'Statistical Modeling' }
  ];
  
  skillKeywords.forEach(skill => {
    if (text.includes(skill.keyword)) {
      analysis.skills.push(skill.display);
      analysis.positionMatch.push(`✅ ${skill.display} skills - matches requirement`);
    }
  });
  
  // 检查认证要求
  const certKeywords = [
    { keyword: 'cfa', display: 'CFA' },
    { keyword: 'cfp', display: 'CFP' },
    { keyword: 'frm', display: 'FRM' },
    { keyword: 'cpa', display: 'CPA' }
  ];
  
  certKeywords.forEach(cert => {
    if (text.includes(cert.keyword)) {
      analysis.certifications.push(cert.display);
      analysis.positionMatch.push(`✅ ${cert.display} certification - matches preferred qualification`);
    }
  });
  ...
  if (text.includes('wholesale') || text.includes('批发银行')) {
    analysis.positionMatch.push('✅ Wholesale banking knowledge - matches requirement');
  }
  
  if (text.includes('business sense') || text.includes('商业意识')) {
    analysis.positionMatch.push('✅ Business sense - matches requirement');
  }
  
  if (text.includes('presentation') || text.includes('演示')) {
    analysis.positionMatch.push('✅ Presentation skills - matches requirement');
  }
    ...
    // 解析工作经历
      ...
      if (companyMatch) {
        ...
        while (j < lines.length && !lines[j].trim().startsWith('### ') && !lines[j].trim().startsWith('## ')) {
          ...
          // 解析职位和持续时间
          if (subLine.startsWith('**') && subLine.includes('|')) {
            const posMatch = subLine.match(/\*\*(.+?)\*\* \| (.+)/);
            if (posMatch) {
              workExp.position = posMatch[1].trim();
              workExp.duration = posMatch[2].trim();
            }
          }
          // 解析主要职责
...
<body>
    <div class="header">
        ...
        <p class="title">Data & Analytics Professional | CFA Charterholder</p>
